...
/**
 * Background service to handle scheduling of starred session notification via {@link android.app.AlarmManager}.
 */
public class SessionAlarmService extends IntentService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {

    private static final String TAG                                   = makeLogTag(SessionAlarmService.class);

    public static final String  ACTION_NOTIFY_SESSION                 = "com.google.samples.apps.iosched.action.NOTIFY_SESSION";
    public static final String  ACTION_NOTIFY_SESSION_FEEDBACK        = "com.google.samples.apps.iosched.action.NOTIFY_SESSION_FEEDBACK";
    public static final String  ACTION_SCHEDULE_FEEDBACK_NOTIFICATION = "com.google.samples.apps.iosched.action.SCHEDULE_FEEDBACK_NOTIFICATION";
    public static final String  ACTION_SCHEDULE_STARRED_BLOCK         = "com.google.samples.apps.iosched.action.SCHEDULE_STARRED_BLOCK";
    public static final String  ACTION_SCHEDULE_ALL_STARRED_BLOCKS    = "com.google.samples.apps.iosched.action.SCHEDULE_ALL_STARRED_BLOCKS";
    public static final String  EXTRA_SESSION_START                   = "com.google.samples.apps.iosched.extra.SESSION_START";
    public static final String  EXTRA_SESSION_END                     = "com.google.samples.apps.iosched.extra.SESSION_END";
    public static final String  EXTRA_SESSION_ALARM_OFFSET            = "com.google.samples.apps.iosched.extra.SESSION_ALARM_OFFSET";
    public static final String  EXTRA_SESSION_ID                      = "com.google.samples.apps.iosched.extra.SESSION_ID";
    ...
    public static final String  EXTRA_SESSION_SPEAKERS                = "com.google.samples.apps.iosched.extra.SESSION_SPEAKERS";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_SCHEDULE_FEEDBACK_NOTIFICATION.equals(action) || ACTION_NOTIFY_SESSION_FEEDBACK.equals(action)) {
            ...
            final String sessionRoom = intent.getStringExtra(SessionAlarmService.EXTRA_SESSION_ROOM);
            final String sessionSpeakers = intent.getStringExtra(SessionAlarmService.EXTRA_SESSION_SPEAKERS);
            if (sessionTitle == null || sessionEnd == UNDEFINED_VALUE || sessionId == null) {
                Log.e(TAG, "Attempted to schedule or notify for feedback without providing extras.");
                return;
            }
            if (ACTION_SCHEDULE_FEEDBACK_NOTIFICATION.equals(action)) {
                LOGD(TAG, "Scheduling feedback alarm for session: " + sessionTitle);
                scheduleFeedbackAlarm(sessionId, sessionEnd, sessionAlarmOffset, sessionTitle, sessionRoom,
                        sessionSpeakers);
            } else {
                LOGD(TAG, "Notifying for feedback on session: " + sessionTitle);
                notifySessionFeedback(sessionId, sessionEnd, sessionTitle, sessionRoom, sessionSpeakers);
            }
            return;
        }
        ...
        if (ACTION_NOTIFY_SESSION.equals(action)) {
            LOGD(TAG,
                    "Notifying about sessions starting at " + sessionStart + " = "
    ...
    public void scheduleFeedbackAlarm(final String sessionId, final long sessionEnd, final long alarmOffset,
            final String sessionTitle, String sessionRoom, String sessionSpeakers) {
        ...
        final Intent feedbackIntent = new Intent(ACTION_NOTIFY_SESSION_FEEDBACK, null, this, SessionAlarmService.class);
        feedbackIntent.setData(new Uri.Builder().authority("com.google.samples.apps.iosched").path(sessionId).build());
    ...
    // concluded.
    private void notifySessionFeedback(final String sessionId, final long sessionEnd, final String sessionTitle,
            final String sessionRoom, final String sessionSpeakers) {
        ...
        if (!PrefUtils.shouldShowSessionFeedbackReminders(this)) {
            LOGD(TAG, "Skipping session feedback notification for session " + sessionId + " (" + sessionTitle
        ...
        final Cursor c = getContentResolver().query(myScheduleUri, MySessionsExistenceQuery.PROJECTION,
        ...
        PendingIntent pi = TaskStackBuilder.create(this).addNextIntent(new Intent(this, MyScheduleActivity.class))
        ...
        NotificationCompat.Builder notifBuilder = new NotificationCompat.Builder(this)
                ...
                .setLights(SessionAlarmService.NOTIFICATION_ARGB_COLOR, SessionAlarmService.NOTIFICATION_LED_ON_MS,
                        SessionAlarmService.NOTIFICATION_LED_OFF_MS).setSmallIcon(R.drawable.ic_stat_notification)
                .setContentIntent(pi).setPriority(Notification.PRIORITY_MAX).setLocalOnly(true) // make it local to the
                                                                                                // phone
    ...
    /**
     * Builds corresponding notification for the Wear device that is paired to this handset. This is done by adding a
     * Data Item to teh Data Store; the Wear device will be notified to build a local notification.
     */
    ...
    private void notifySession(final long sessionStart, final long alarmOffset) {
        ...
        // Avoid repeated notifications.
        if (alarmOffset == UNDEFINED_ALARM_OFFSET
                && UIUtils.isNotificationFiredForBlock(this,
        ...
        if (starredCount == 1) {
            taskBuilder.addNextIntent(new Intent(Intent.ACTION_VIEW, ScheduleContract.Sessions
                    .buildSessionUri(singleSessionId)));
        }
        ...
        NotificationCompat.Builder notifBuilder = new NotificationCompat.Builder(this)
                ...
                .setLights(SessionAlarmService.NOTIFICATION_ARGB_COLOR, SessionAlarmService.NOTIFICATION_LED_ON_MS,
                        SessionAlarmService.NOTIFICATION_LED_OFF_MS).setSmallIcon(R.drawable.ic_stat_notification)
                .setContentIntent(pi).setPriority(Notification.PRIORITY_MAX).setAutoCancel(true);
        if (minutesLeft > 5) {
    ...
    private PendingIntent createRoomMapIntent(final String roomId) {
        Intent mapIntent = new Intent(getApplicationContext(), UIUtils.getMapActivityClass(getApplicationContext()));
    ...
    private void scheduleAllStarredBlocks() {
        final ContentResolver cr = getContentResolver();
        final Cursor c = cr.query(ScheduleContract.Sessions.CONTENT_MY_SCHEDULE_URI, new String[] {
                "distinct " + ScheduleContract.Sessions.SESSION_START, ScheduleContract.Sessions.SESSION_END,
                ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE}, null, null, null);
        if (c == null) {
            return;
        }
    ...
    private void scheduleAllStarredSessionFeedbacks() {
        ...
        final Cursor c = cr.query(ScheduleContract.Sessions.CONTENT_MY_SCHEDULE_URI, new String[] {
                ScheduleContract.Sessions.SESSION_ID, ScheduleContract.Sessions.SESSION_TITLE,
                ScheduleContract.Sessions.SESSION_END, ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE,
                ScheduleContract.Sessions.ROOM_NAME, ScheduleContract.Sessions.SESSION_SPEAKER_NAMES,}, null, null,
                null);
        if (c == null) {
            return;
        }
        while (c.moveToNext()) {
            ...
            scheduleFeedbackAlarm(sessionId, sessionEnd, UNDEFINED_ALARM_OFFSET, sessionTitle, sessionRoom,
                    sessionSpeakers);
        }
    }
    ...
    public interface MySessionsExistenceQuery {
        ...
        String[]                   PROJECTION   = {ScheduleContract.MySchedule.SESSION_ID};

        int                        SESSION_ID   = 0;

        public static final String WHERE_CLAUSE = ScheduleContract.MySchedule.SESSION_ID + "=?";
    }
