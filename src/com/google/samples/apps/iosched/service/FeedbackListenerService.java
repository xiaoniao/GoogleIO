...
/**
 * A {@link com.google.android.gms.wearable.WearableListenerService} service to receive the session feedback from the
 * wearable device and handle dismissal of notifications by deleting the associated Data Items.
 */
public class FeedbackListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks,
    ...
    private static final long   TIMEOUT_S     = 10;                                       // how long wait for APi
    ...
    /**
     * Removes the Data Item that was used to create a notification on the watch. By deleting the data item, a
     * {@link com.google.android.gms.wearable.WearableListenerService} on the watch will be notified and the
     * notification on the watch will be removed.
     * <p/>
     * Since connection to the Google API client is asynchronous, we spawn a thread and wait for the connection to be
     * established before attempting to use the Google API client.
     * 
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (mGoogleApiClient.isConnected()) {
                    Wearable.DataApi.deleteDataItems(mGoogleApiClient, putDataMapRequest.getUri()).setResultCallback(
    ...
    /**
     * Persisting the feedback in the database. The input is the JSON string that represents the response from the user
     * on the paired wear device. The format of a typical response is:
     * 
     * <pre>
    ...
    private void saveFeedback(String jsonString) {
        try {
            ...
            if (null != jsonArray) {
                ...
                FeedbackUtils
                        .saveSessionFeedback(this, sessionId, answers[0], answers[1], answers[2], answers[3], null);
            }
