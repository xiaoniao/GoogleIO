...
public class ScheduleProvider extends ContentProvider {
    ...
    /** 生成查询问号占位符 **/
    private String makeQuestionMarkTuple(int count) { // 3 returns (?,?,?)
        if (count < 1) {
            return "()";
        }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (match) {
            default: {
                ...
                boolean distinct = !TextUtils.isEmpty(uri.getQueryParameter(ScheduleContract.QUERY_PARAMETER_DISTINCT));
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            ...
            case MY_SCHEDULE: {
                ...
                notifyChange(uri);
                return Sessions.buildSessionUri(values.getAsString(ScheduleContract.MyScheduleColumns.SESSION_ID));
            }
            case SPEAKERS: {
    ...
    private SelectionBuilder buildSimpleSelection(Uri uri) {
        ...
        switch (match) {
            ...
            case SESSIONS_MY_SCHEDULE: {
                final String sessionId = Sessions.getSessionId(uri);
                return builder.table(Tables.MY_SCHEDULE).where(ScheduleContract.MyScheduleColumns.SESSION_ID + "=?",
                        sessionId);
            }
            case MY_SCHEDULE: {
            ...
            case EXPERTS_ID: {
                ...
                return builder.table(Tables.EXPERTS).where(Experts.EXPERT_ID + "= ?", expertId);
            }
            case HASHTAGS: {
            ...
            case PEOPLE_IVE_MET_ID: {
                ...
                return builder.table(Tables.PEOPLE_IVE_MET).where(PeopleIveMet.PERSON_ID + "=?", personId);
            }
            case VIDEOS: {
    ...
    private SelectionBuilder buildExpandedSelection(Uri uri, int match) {
        ...
        switch (match) {
            ...
            case ROOMS_ID_SESSIONS: {
                ...
                final String roomId = Rooms.getRoomId(uri);
                return builder.table(Tables.SESSIONS_JOIN_ROOMS).mapToTable(Sessions._ID, Tables.SESSIONS).mapToTable(Sessions.ROOM_ID, Tables.SESSIONS).where(Qualified.SESSIONS_ROOM_ID + "=?", roomId);
            ...
            case SESSIONS_UNSCHEDULED: {
                ...
                return builder.table(Tables.SESSIONS_JOIN_ROOMS_TAGS_FEEDBACK_MYSCHEDULE, getCurrentAccountName(uri, true)).mapToTable(Sessions._ID, Tables.SESSIONS).mapToTable(Sessions.ROOM_ID, Tables.SESSIONS).mapToTable(Sessions.SESSION_ID, Tables.SESSIONS).map(Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)").where(Sessions.SESSION_IN_MY_SCHEDULE + "=0").where(Sessions.SESSION_START + ">=?", String.valueOf(interval[0])).where(Sessions.SESSION_START + "<?", String.valueOf(interval[1])).groupBy(Qualified.SESSIONS_SESSION_ID);
            }
            
            case SESSIONS_SEARCH: {
                
                final String query = Sessions.getSearchQuery(uri);
                return builder.table(Tables.SESSIONS_SEARCH_JOIN_SESSIONS_ROOMS, getCurrentAccountName(uri, true)).map(Sessions.SEARCH_SNIPPET, Subquery.SESSIONS_SNIPPET).mapToTable(Sessions._ID, Tables.SESSIONS).mapToTable(Sessions.SESSION_ID, Tables.SESSIONS).mapToTable(Sessions.ROOM_ID, Tables.SESSIONS).map(Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)").where(SessionsSearchColumns.BODY + " MATCH ?", query);
            ...
            case SESSIONS_ROOM_AFTER: {
                ...
                return builder.table(Tables.SESSIONS_JOIN_ROOMS, getCurrentAccountName(uri, true)).mapToTable(Sessions._ID, Tables.SESSIONS).mapToTable(Sessions.ROOM_ID, Tables.SESSIONS).where(Qualified.SESSIONS_ROOM_ID + "=?", room).where("(" + Sessions.SESSION_START + "<= ? AND " + Sessions.SESSION_END + " >= ?) OR (" + Sessions.SESSION_START + " >= ?)", time, time, time);
            ...
            case MY_SCHEDULE: {
                ...
                return builder.table(Tables.MY_SCHEDULE).where(MySchedule.MY_SCHEDULE_ACCOUNT_NAME + "=?",getCurrentAccountName(uri, true));
    ...
    private interface Subquery {
        ...
        String SESSION_HAS_GIVEN_FEEDBACK = "(SELECT COUNT(1) FROM " + Tables.FEEDBACK + " WHERE " + Qualified.FEEDBACK_SESSION_ID + "=" + Qualified.SESSIONS_SESSION_ID + ")";
    ...
    private interface Qualified {
        ...
        String SESSIONS_TAGS_SESSION_ID     = Tables.SESSIONS_TAGS + "." + ScheduleDatabase.SessionsTags.SESSION_ID;
