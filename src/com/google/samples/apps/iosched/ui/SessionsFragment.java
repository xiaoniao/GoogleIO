...
/*
 * A {@link ListFragment} showing a list of sessions. The fragment arguments indicate what is the list of sessions to
 * show. It may be a set of tag filters or a search query.
 */
public class SessionsFragment extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>,
    ...
    public void onAttach(Activity activity) {
        ...
        // 主Activity必须实现Callbacks接口，否则抛出异常
        ...
        activity.getContentResolver().registerContentObserver(ScheduleContract.Tags.CONTENT_URI, true, mTagsObserver);
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle data) {
        ...
        String liveStreamedOnlySelection = UIUtils.shouldShowLiveSessionsOnly(getActivity()) ? "IFNULL("
                + ScheduleContract.Sessions.SESSION_LIVESTREAM_URL + ",'')!=''" : null;
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        LOGD(TAG, "Loader finished: "
                + (token == SessionsQuery.NORMAL_TOKEN ? "sessions" : token == SessionsQuery.SEARCH_TOKEN ? "search"
    ...
    private final SharedPreferences.OnSharedPreferenceChangeListener mPrefChangeListener = new SharedPreferences.OnSharedPreferenceChangeListener() {
                                                                                             ...
                                                                                             public void onSharedPreferenceChanged(
                                                                                                     ...
                                                                                                     String key) {
                                                                                                 if (isAdded()) {
                                                                                                     if (PrefUtils.PREF_LOCAL_TIMES
                                                                                                             .equals(key)) {
                                                                                                         updateCollectionView();
                                                                                                     } else if (PrefUtils.PREF_ATTENDEE_AT_VENUE
                                                                                                             .equals(key)) {
                                                                                                         if (mCursor != null) {
    ...
    private void updateCollectionView() {
        ...
        if (mCursor == null || mTagMetadata == null) {
            ...
            LOGD(TAG, "updateCollectionView: not ready yet... " + (mCursor == null ? "no cursor." : "no tag metadata."));
            // not ready!
            return;
        }
        ...
        LOGD(TAG,
                "Updating CollectionView with inventory, # groups = " + inv.getGroupCount() + " total items = "
    ...
    private void showEmptyView() {
        final String searchQuery = ScheduleContract.Sessions.isSearchUri(mCurrentUri) ? ScheduleContract.Sessions
    ...
    private CollectionView.Inventory prepareInventory() {
        ...
        final int displayCols = getResources().getInteger(
                expandedMode ? R.integer.explore_2nd_level_grid_columns : R.integer.explore_1st_level_grid_columns);
        ...
        while (mCursor.moveToNext()) {
            ...
            long sessionEnd = mCursor.getLong(mCursor.getColumnIndex(ScheduleContract.Sessions.SESSION_END));
            ...
            if (expandedMode) {
                ...
                TagMetadata.Tag groupTag = tags == null ? null : mTagMetadata.getSessionGroupTag(tags.split(","));
                if (groupTag != null) {
            ...
            LOGV(TAG, "Data item #" + dataIndex + ", sessionEnd=" + sessionEnd + ", groupLabel=" + groupLabel
                    + " showAsPast=" + showAsPast);
            ...
            if (!useExpandedMode() && !showAsPast && heroGroup == null) {
                ...
                group = heroGroup = new CollectionView.InventoryGroup(HERO_GROUP_ID).setDisplayCols(1) // hero item
                                                                                                       // spans all
                                                                                                       // columns
                ...
                if (!map.containsKey(groupLabel)) {
                    ...
                    group = new CollectionView.InventoryGroup(nextGroupId++).setDisplayCols(displayCols)
                            .setShowHeader(!TextUtils.isEmpty(groupLabel)).setHeaderLabel(groupLabel);
    ...
    public void bindCollectionItemView(Context context, View view, int groupId, int indexInGroup, int dataIndex,
        ...
        if (subtitleView != null) {
            ...
            shortSubtitleView.setText(UIUtils.formatSessionSubtitle(sessionStart, sessionEnd, roomName, mBuffer,
                    context, true) + liveNowText);
        }
    ...
    private boolean setupMessageCard(View hero) {
        ...
        if (!PrefUtils.hasAnsweredLocalOrRemote(getActivity()) && !TimeUtils.hasConferenceEnded(getActivity())) {
    ...
    private void setupLocalOrRemoteCard(final MessageCardView card) {
        ...
        card.setButton(0, getString(R.string.attending_remotely), CARD_ANSWER_ATTENDING_REMOTELY, false, 0);
        card.setButton(1, getString(R.string.attending_in_person), CARD_ANSWER_ATTENDING_IN_PERSON, true, 0);
    ...
    private void setupWifiOfferCard(final MessageCardView card) {
        ...
        card.setButton(0, getString(R.string.no_thanks), CARD_ANSWER_NO, false, 0);
        card.setButton(1, getString(R.string.setup_wifi_yes), CARD_ANSWER_YES, true, 0);
        final Context context = getActivity().getApplicationContext();
        card.setListener(new MessageCardView.OnMessageCardButtonClicked() {
            @Override
            public void onMessageCardButtonClicked(final String tag) {
    ...
    private void setupIOExtendedCard(final MessageCardView card) {
        ...
        card.setButton(0, getString(R.string.no_thanks), CARD_ANSWER_NO, false, 0);
        card.setButton(1, getString(R.string.browse_events), CARD_ANSWER_YES, true, 0);
        ...
        card.setListener(new MessageCardView.OnMessageCardButtonClicked() {
            ...
            public void onMessageCardButtonClicked(final String tag) {
                ...
                mHandler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (CARD_ANSWER_YES.equals(tag)) {
                            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(Config.IO_EXTENDED_LINK));
    ...
    private interface SessionsQuery {
        ...
        String[] NORMAL_PROJECTION = {BaseColumns._ID, ScheduleContract.Sessions.SESSION_ID,
                                           ScheduleContract.Sessions.SESSION_TITLE,
                                           ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE,
                                           ScheduleContract.Sessions.SESSION_START,
                                           ScheduleContract.Sessions.SESSION_END, ScheduleContract.Rooms.ROOM_NAME,
                                           ScheduleContract.Rooms.ROOM_ID, ScheduleContract.Sessions.SESSION_HASHTAG,
        ...
        String[] SEARCH_PROJECTION = {BaseColumns._ID, ScheduleContract.Sessions.SESSION_ID,
                                           ScheduleContract.Sessions.SESSION_TITLE,
                                           ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE,
                                           ScheduleContract.Sessions.SESSION_START,
                                           ScheduleContract.Sessions.SESSION_END, ScheduleContract.Rooms.ROOM_NAME,
