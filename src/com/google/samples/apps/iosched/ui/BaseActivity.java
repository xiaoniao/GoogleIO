...
/**
 * A base activity that handles common functionality in the app. This includes the navigation drawer, login and
 * authentication, Action Bar tweaks, amongst others.
 */
public abstract class BaseActivity extends Activity implements LoginAndAuthHelper.Callbacks,
    ...
    private static final int[]                             NAVDRAWER_TITLE_RES_ID           = new int[] {
            R.string.navdrawer_item_my_schedule, R.string.navdrawer_item_explore, R.string.navdrawer_item_map,
            R.string.navdrawer_item_social, R.string.navdrawer_item_video_library, R.string.navdrawer_item_sign_in,
            R.string.navdrawer_item_settings, R.string.navdrawer_item_experts_directory,
    ...
    private static final int[]                             NAVDRAWER_ICON_RES_ID            = new int[] {
            ...
            0, // Sign in
            R.drawable.ic_drawer_settings, R.drawable.ic_drawer_experts, R.drawable.ic_drawer_people_met,};
    ...
    private void trySetupSwipeRefresh() {
        ...
        if (mSwipeRefreshLayout != null) {
            mSwipeRefreshLayout.setColorScheme(R.color.refresh_progress_1, R.color.refresh_progress_2,
    ...
    /**
     * Returns the navigation drawer item that corresponds to this Activity. Subclasses of BaseActivity override this to
     * indicate what nav drawer item corresponds to them Return NAVDRAWER_ITEM_INVALID to mean that this Activity should
     * not have a Nav Drawer.
     */
    ...
    /**
     * Sets up the navigation drawer as appropriate. Note that the nav drawer will be different depending on whether the
     * attendee indicated that they are attending the event on-site vs. attending remotely.
     */
    ...
    /**
     * Sets up the given navdrawer item's appearance to the selected state. Note: this could also be accomplished
     * (perhaps more cleanly) with state-based layouts.
     */
    ...
    /**
     * Sets up the account box. The account box is the area at the top of the nav drawer that shows which account the
     * user is logged in as, and lets them switch accounts. It also shows the user's Google+ cover photo as background.
     */
    ...
    private void setupAccountBoxToggle() {
        ...
        if (mAccountBoxExpanded) {
            ...
            subSet.playTogether(
                    ObjectAnimator.ofFloat(mAccountListContainer, View.ALPHA, 1).setDuration(
                            ACCOUNT_BOX_EXPAND_ANIM_DURATION),
                    ObjectAnimator.ofFloat(mAccountListContainer, View.TRANSLATION_Y, 0).setDuration(
                            ACCOUNT_BOX_EXPAND_ANIM_DURATION));
            set.playSequentially(
                    ObjectAnimator.ofFloat(mDrawerItemsListContainer, View.ALPHA, 0).setDuration(
            ...
            AnimatorSet subSet = new AnimatorSet();
            subSet.playTogether(
                    ObjectAnimator.ofFloat(mAccountListContainer, View.ALPHA, 0).setDuration(
                            ACCOUNT_BOX_EXPAND_ANIM_DURATION),
                    ObjectAnimator.ofFloat(mAccountListContainer, View.TRANSLATION_Y, hideTranslateY).setDuration(
                            ACCOUNT_BOX_EXPAND_ANIM_DURATION));
            set.playSequentially(
                    subSet,
                    ObjectAnimator.ofFloat(mDrawerItemsListContainer, View.ALPHA, 1).setDuration(
                            ACCOUNT_BOX_EXPAND_ANIM_DURATION));
            set.start();
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            ...
            case R.id.menu_io_extended:
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(Config.IO_EXTENDED_LINK)));
                break;
    ...
    private void launchIoHunt() {
        if (!TextUtils.isEmpty(Config.IO_HUNT_PACKAGE_NAME)) {
            ...
            if (launchIntent != null) {
                ...
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(Config.PLAY_STORE_URL_PREFIX
    ...
    /**
     * Performs the one-time data bootstrap. This means taking our prepackaged conference data from the
     * R.raw.bootstrap_data resource, and parsing it to populate the database. This data contains the sessions,
     * speakers, etc.
     */
    ...
    /**
     * Indicates that the main content has scrolled (for the purposes of showing/hiding the action bar for the
     * "action bar auto hide" effect). currentY and deltaY may be exact (if the underlying view supports it) or may be
     * approximate indications: deltaY may be INT_MAX to mean "scrolled forward indeterminately" and INT_MIN to mean
     * "scrolled backward indeterminately". currentY may be 0 to mean "somewhere close to the start of the
     * list" and INT_MAX to mean "we don't know, but not at the start of the list"
     */
    ...
    protected void enableActionBarAutoHide(final ListView listView) {
        ...
        listView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                onMainContentScrolled(firstVisibleItem <= ITEMS_THRESHOLD ? 0 : Integer.MAX_VALUE, lastFvi
                        - firstVisibleItem > 0 ? Integer.MIN_VALUE : lastFvi == firstVisibleItem ? 0
                        : Integer.MAX_VALUE);
    ...
    private void registerGCMClient() {
        ...
        if (TextUtils.isEmpty(regId)) {
            ...
            final String gcmKey = AccountUtils.hasActiveAccount(this) ? AccountUtils.getGcmKey(this,
                    AccountUtils.getActiveAccountName(this)) : null;
    ...
    private void updateStatusBarForNavDrawerSlide(float slideOffset) {
        ...
        mLPreviewUtils.setStatusBarColor((Integer)ARGB_EVALUATOR.evaluate(slideOffset, mThemedStatusBarColor,
                Color.BLACK));
    }
