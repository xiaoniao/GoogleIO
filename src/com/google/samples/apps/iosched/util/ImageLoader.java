...
public class ImageLoader {
    ...
    /**
     * Load an image from a url into an ImageView using the default placeholder drawable if available.
     * 
     ...
     * @param imageView The target ImageView to load the image into.
     * @param requestListener A listener to monitor the request result.
     * @param placeholderOverride A drawable to use as a placeholder for this specific image. If this parameter is
     *        present, {@link #mPlaceHolderResId} if ignored for this request.
     */
    public void loadImage(String url, ImageView imageView, RequestListener<String> requestListener,
            Drawable placeholderOverride, boolean crop) {
        DrawableRequestBuilder<String> request = beginImageLoad(url, requestListener, crop).animate(
    ...
    public DrawableRequestBuilder<String> beginImageLoad(String url, RequestListener<String> requestListener,
            boolean crop) {
        return mGlideModelRequest.load(url).listener(requestListener).bitmapTransform(crop ? mCenterCrop : mNone);
    }

    /**
     * Load an image from a url into the given image view using the default placeholder if available.
     * 
     * @param url The web URL of an image.
     * @param imageView The target ImageView to load the image into.
     */
    ...
    private static class VariableWidthImageLoader extends BaseGlideUrlLoader<String> {
        ...
        /**
         * If the URL contains a special variable width indicator (eg "__w-200-400-800__") we get the buckets from the
         * URL (200, 400 and 800 in the example) and replace the URL with the best bucket for the requested width (the
         * bucket immediately larger than the requested width).
         */
        @Override
