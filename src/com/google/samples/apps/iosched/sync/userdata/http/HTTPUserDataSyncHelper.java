...
/**
 * Helper class that syncs starred sessions data with Drive's AppData folder using direct HTTP Drive API through
...
public class HTTPUserDataSyncHelper extends AbstractUserDataSyncHelper {
    ...
    public HTTPUserDataSyncHelper(Context context, String accountName) {
        ...
        mCredentials = GoogleAccountCredential
                .usingOAuth2(mContext, java.util.Arrays.asList(DriveScopes.DRIVE_APPDATA));
     ...
     * 
     * Synchronization steps: 1. If there are local changes, sync the latest local version with remote and ignore merge
     * conflicts. The last write wins. 2. If there are no local changes, fetch the latest remote version. If it includes
     * changes, notify that preferences have changed.
     */
    ...
    /**
     * Constructs a Drive service in the current context and with the credentials use to initiate AppdataPreferences
     * instance.
     * 
     ...
     * @throws IOException
     */
    private Set<String> mergeDirtyActions(List<UserAction> actions, Set<String> starredSessions) throws IOException {
        // apply "dirty" actions:
    ...
    /**
     * Handles API exceptions and notifies OnExceptionListener if given exception is a UserRecoverableAuthIOException.
     * 
