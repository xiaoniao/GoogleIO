...
/**
 * Helper class that parses conference data and imports them into the app's Content Provider.
 */
public class ConferenceDataHandler {
    ...
    private static final String[] DATA_KEYS_IN_ORDER             = {DATA_KEY_ROOMS, DATA_KEY_BLOCKS, DATA_KEY_TAGS,
            DATA_KEY_SPEAKERS, DATA_KEY_SESSIONS, DATA_KEY_SEARCH_SUGGESTIONS, DATA_KEY_MAP, DATA_KEY_HASHTAGS,
            DATA_KEY_EXPERTS, DATA_KEY_VIDEOS, DATA_KEY_PARTNERS };
    ...
    /**
     * Parses the conference data in the given objects and imports the data into the content provider.
     * 
     * The format of the data is documented at https://code.google.com/p/iosched.
     * 
     ...
     */
    public void applyConferenceData(String[] dataBodies, String dataTimestamp, boolean downloadsAllowed)
            throws IOException {
        ...
        // process the jsons. This will call each of the handlers when appropriate to deal with the objects we see in
        // the data.
    ...
    /**
     * Processes a conference data body and calls the appropriate data type handlers to process each of the objects
     * represented therein.
     * 
    ...
    private void processDataBody(String dataBody) throws IOException {
        ...
        try {
            ...
            reader.beginObject();// the whole file is a single JSON object

            while (reader.hasNext()) {

                String key = reader.nextName();// the key is "rooms", "speakers", "tracks", etc.

                if (mHandlerForKey.containsKey(key)) {

                    mHandlerForKey.get(key).process(parser.parse(reader)); // pass the value to the corresponding
                                                                           // handler
                } else {

                    reader.skipValue(); // Skipping unknown key in conference data json:
                }
            }
            reader.endObject();
        } finally {
            ...
            if (!MapUtils.hasTile(mContext, filename)) {
                ...
                if (MapUtils.hasTileAsset(mContext, filename)) {
                    ...
                    try {
                        ...
                        // SVG svg = new SVGBuilder().readFromInputStream(is).build(); ×¢ÊÍµô
                        is.close();
