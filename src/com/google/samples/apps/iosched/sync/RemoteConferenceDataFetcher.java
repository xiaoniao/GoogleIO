...
public class RemoteConferenceDataFetcher {
     ...
     * 
     * @param refTimestamp The timestamp of the data to use as a reference; if the remote data is not newer than this
     *        timestamp, no data will be downloaded and this method will return null.
     * 
    ...
    public String[] fetchConferenceDataIfNewer(String refTimestamp) throws IOException {
        ...
        if (status == HttpURLConnection.HTTP_OK) {
            ...
            if (TextUtils.isEmpty(body)) {
                ...
                throw new IOException("Error fetching conference data manifest: no data."); // --------->>怎么处处是异常抛出,有什么特殊用途
            }
    ...
    /**
     * Returns the remote manifest file's URL. This is stored as a resource in the app, but can be overriden by a file
     * in the filesystem for debug purposes.
     * 
     ...
     * 从网络缓存中获取数据
     * 
     * Fetches a file from the cache/network, from an absolute or relative URL. If the file is available in our cache,
     * we read it from there; if not, we will download it from the network and cache it.
     * 
     * @param url The URL to fetch the file from. The URL may be absolute or relative; if relative, it will be
     *        considered to be relative to the manifest URL.
    ...
    private String fetchFile(String url) throws IOException {
        ...
        if (response.getStatus() == HttpURLConnection.HTTP_OK) {
            ...
            if (TextUtils.isEmpty(body)) {
                throw new IOException("Got empty response when attempting to fetch " + sanitizeUrl(url));
            }
            ...
            throw new IOException("Request for URL " + sanitizeUrl(url) + " failed with HTTP error "
                    + response.getStatus());
        }
    }
     ...
     * @return The cached response corresponding to the URL; or null if the given URL does not exist in our cache.
    ...
    /**
     * Returns the cache key to be used to store the given URL. The cache key is the file name under which the contents
     * of the URL are stored.
     * 
     * @param url The URL.
    ...
    // Sanitize a URL for logging purposes (only the last component is left visible). 过滤URL，只留下最后一部分
    private String sanitizeUrl(String url) {
        int i = url.lastIndexOf('/');
        if (i >= 0 && i < url.length()) {
            return url.substring(0, i).replaceAll("[A-za-z]", "*") + url.substring(i);
        } else
     ...
     * @return The contents of the set of files referenced from the manifest, or null if none could be retrieved.
    ...
    private String[] processManifest(String manifestJson) throws IOException {
        ...
        // 卧槽,请求的数据转换成了DataManifest了，并返回了DataManifest中的数组字符串

        LOGD(TAG, "Processing data manifest, length " + manifestJson.length());

        DataManifest manifest = new Gson().fromJson(manifestJson, DataManifest.class); // GSON 转换为实体类
    ...
    // Delete unnecessary files from our cache 清除缓存
