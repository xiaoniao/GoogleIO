...
public class SyncHelper {
    ...
    private ConferenceDataHandler       mConferenceDataHandler;        // 操作Handler解析数据并保存到ContentProviderOperate中
    private RemoteConferenceDataFetcher mRemoteDataFetcher;            // 下载数据>使用了网络缓存机制

    public SyncHelper(Context context) {
        mContext = context;
        mConferenceDataHandler = new ConferenceDataHandler(mContext); // 实例化
        mRemoteDataFetcher = new RemoteConferenceDataFetcher(mContext);// 实例化
    }
    ...
    public static void requestManualSync(Account mChosenAccount, boolean userDataSyncOnly) {
        ...
        if (mChosenAccount != null) {
            ...
            boolean pending = ContentResolver.isSyncPending(mChosenAccount, ScheduleContract.CONTENT_AUTHORITY);

            if (pending) {
                LOGD(TAG, "Warning: sync is PENDING. Will cancel.");
            }

            boolean active = ContentResolver.isSyncActive(mChosenAccount, ScheduleContract.CONTENT_AUTHORITY);
    ...
    /**
     * Attempts to perform conference data synchronization. The data comes from the remote URL configured in
     * {@link com.google.samples.apps.iosched.Config#MANIFEST_URL}. The remote URL must point to a manifest file that,
     * in turn, can reference other files. For more details about conference data synchronization, refer to the
     * documentation at http://code.google.com/p/iosched.
     * 
    ...
    public boolean performSync(SyncResult syncResult, Account account, Bundle extras) {
        ...
        if (!manualSync && timeSinceAttempt >= 0 && timeSinceAttempt < Config.MIN_INTERVAL_BETWEEN_SYNCS) {
            /*
             * Code removed because it was causing a runaway sync; probably because we are setting syncResult.delayUntil
             * incorrectly.
             * 
             * Random r = new Random(); long toWait = 10000 + r.nextInt(30000) // random jitter between 10 - 40 seconds
             * + Config.MIN_INTERVAL_BETWEEN_SYNCS - timeSinceAttempt; LOGW(TAG,
             * "Sync throttled!! Another sync was attempted just " + timeSinceAttempt + "ms ago. Requesting delay of " +
             * toWait + "ms."); syncResult.fullSyncRequested = true; syncResult.delayUntil = (System.currentTimeMillis()
             * + toWait) / 1000L; return false;
             */
        }
        ...
        int[] opsToPerform = userDataOnly ? new int[] {OP_USER_SCHEDULE_SYNC} : new int[] {OP_REMOTE_SYNC,
                OP_USER_SCHEDULE_SYNC, OP_USER_FEEDBACK_SYNC};
    ...
    public static void performPostSyncChores(final Context context) {
        ...
        context.getContentResolver().update(ScheduleContract.SearchIndex.CONTENT_URI, new ContentValues(), null, null);
    ...
    /**
     * Checks if the remote server has new data that we need to import. If so, download the new data and import it into
     * the database.
     * 
    ...
    private boolean doRemoteSync() throws IOException {
        ...
        String[] dataFiles = mRemoteDataFetcher.fetchConferenceDataIfNewer(mConferenceDataHandler.getDataTimestamp());

        if (dataFiles != null) {
            LOGI(TAG, "Applying remote data.");
            // save the remote data to the database
            mConferenceDataHandler.applyConferenceData(dataFiles, mRemoteDataFetcher.getServerDataTimestamp(), true);
    ...
    private boolean doUserScheduleSync(String accountName) throws IOException {
        ...
        AbstractUserDataSyncHelper helper = UserDataSyncHelperFactory.buildSyncHelper(mContext, accountName);
        boolean modified = helper.sync();
        if (modified) {
            // schedule notifications for the starred sessions
            Intent scheduleIntent = new Intent(SessionAlarmService.ACTION_SCHEDULE_ALL_STARRED_BLOCKS, null, mContext,
    ...
    private boolean clearExpertsIfNecessary() {
        if (Config.hasExpertsDirectoryExpired()) {
            return 0 < mContext.getContentResolver().delete(ScheduleContract.Experts.CONTENT_URI, null, null);
        }
        return false;
    }
    ...
    public static void updateSyncInterval(final Context context, final Account account) {
        ...
        if (recommended != current) {
            ...
            ContentResolver.addPeriodicSync(account, ScheduleContract.CONTENT_AUTHORITY, new Bundle(),
                    recommended / 1000L);
