...
public class ScheduleHelper {
    ...
    protected void setSessionCounters(ArrayList<ScheduleItem> items, long dayStart, long dayEnd) {
        ...
        String liveStreamedOnlySelection = UIUtils.shouldShowLiveSessionsOnly(mContext) ? "AND IFNULL("
        ...
        while (it.hasNext()) {
            ...
            if (i.type == ScheduleItem.FREE) {
                if (i.endTime < now) {
                    ...
                    i.subtitle = mContext.getResources().getQuantityString(R.plurals.schedule_block_subtitle,
                            i.numOfSessions, i.numOfSessions);
                }
    ...
    /** 异步获取数据 **/
    public void getScheduleDataAsync(final MyScheduleAdapter adapter, long start, long end) {
        ...
        new AsyncTask<Long, Void, ArrayList<ScheduleItem>>() {
            ...
            protected ArrayList<ScheduleItem> doInBackground(Long... params) {
                Long start = params[0];
                Long end = params[1];
                return getScheduleData(start, end); // 异步获取数据
            }

            @Override
            protected void onPostExecute(ArrayList<ScheduleItem> scheduleItems) {
                adapter.updateItems(scheduleItems); // 刷新适配器
            }
        }.execute(start, end);
    }

    /** 添加Sessions **/
    protected void addSessions(long start, long end, ArrayList<ScheduleItem> mutableItems, ArrayList<ScheduleItem> immutableItems) {

        Cursor cursor = mContext.getContentResolver().query(
                ScheduleContract.addOverrideAccountName(Sessions.CONTENT_MY_SCHEDULE_URI,
                        AccountUtils.getActiveAccountName(mContext)), SessionsQuery.PROJECTION,
        ...
        while (cursor.moveToNext()) {
            ...
            item.subtitle = UIUtils.formatSessionSubtitle(cursor.getString(SessionsQuery.ROOM_ROOM_NAME),
    ...
    /** 添加Blocks  **/
    protected void addBlocks(long start, long end, ArrayList<ScheduleItem> mutableItems, ArrayList<ScheduleItem> immutableItems) {
        
        /** 妹的，原来是通过ContentProvider获得的 **/
        Cursor cursor = mContext.getContentResolver().query(Blocks.CONTENT_URI, BlocksQuery.PROJECTION, Blocks.BLOCK_START + " >= ? and " + Blocks.BLOCK_START + " <= ?",new String[] {String.valueOf(start), String.valueOf(end)}, Blocks.BLOCK_START);
    ...
    private interface SessionsQuery {
        String[] PROJECTION             = {Sessions.SESSION_ID, Sessions.SESSION_TITLE, Sessions.SESSION_START,
                                                Sessions.SESSION_END, ScheduleContract.Rooms.ROOM_NAME,
                                                Sessions.SESSION_IN_MY_SCHEDULE, Sessions.SESSION_LIVESTREAM_URL,
                                                Sessions.SESSION_SPEAKER_NAMES, Sessions.SESSION_PHOTO_URL,
                                                Sessions.SESSION_COLOR, Sessions.HAS_GIVEN_FEEDBACK,};
    ...
    private interface BlocksQuery {
        String[] PROJECTION     = {Blocks.BLOCK_TITLE, Blocks.BLOCK_TYPE, Blocks.BLOCK_START, Blocks.BLOCK_END,
    ...
    private interface SessionsCounterQuery {
        String[] PROJECTION             = {Sessions.SESSION_START, Sessions.SESSION_END,
                                                Sessions.SESSION_INTERVAL_COUNT, Sessions.SESSION_IN_MY_SCHEDULE,};
